...
def play(game_status, x_or_o, coordinate): #game_status 가 x,o의 위치 이때까지 놓인 위치들, coordination 은 좌표 
...
def determine_if_o_wins(game-stauts, winning_positions):
    ...
    for y in range(y_size):
        ...
        for z in range(y_cell_size):
            for x in range(x_size):
                if z == 1:
                    if (x, y) in game_status['x_postions']:
